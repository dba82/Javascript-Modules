...
export function unitVector(){
    let a = [1,0];
    return vec.rotate2d(a, number(0, 2*Math.PI));
}

export function vector(minLength, maxLength, direction?){
    return vec.withLength(number(minLength, maxLength) , direction || unitVector())
}

export function point(x, y, w,h){
    if (!w && !h){
        h = y;
        w = x;
        x = 0;
...
export function rect(canvas_width, canvas_height, w=20, h=20 ){
    return [integer(0,canvas_width),integer(0,canvas_height), w, h]
}

export function pick(array, putback = true){
    let i = integer(0, array.length);
    let result = array[i];
    if (!putback){
        array.splice(i, 1);
    }
...
export function color(brightness = .5, saturation = 1, alpha = 1.0){
